...
const _navigationOptions = reactive({
  ...
  lastVisitedSections: {},
  isSidebarExpanded:
    _savedState.isSidebarExpanded !== undefined
      ? _savedState.isSidebarExpanded
      : true,
});
...
export function useNavigation() {
  ...
  const isSectionVisible = (sectionId) => {
    return (
      _navigationOptions.activeSectionId === sectionId ||
  ...
  const toggleSidebarStatus = () => {
    _navigationOptions.isSidebarExpanded =
      !_navigationOptions.isSidebarExpanded;
    _saveState();
  };
  ...
  const getSectionCategory = (sectionId) => {
    for (let categoryId in _categorySectionMapping) {
      ...
      if (categorySectionIds.includes(sectionId)) return categoryId;
    }
  ...
  const getActiveCategoryId = () => {
    ...
    Object.entries(_categorySectionMapping).forEach(
      ([categoryId, sectionArray]) => {
